...
class lab2_straight:
    ...
    def callback(self, msg):
        ...
        if msg.state == "LANE_FOLLOWING":
            ...
            self.straight = Twist2DStamped(self.header, self.velocity, self.omega)
            ...
            while self.curr_secs < self.header.stamp.secs:
                ...
                if self.curr_secs == self.goal_secs:
                    ...
                    self.straight = Twist2DStamped(self.header, self.velocity, self.omega)
            self.pub.publish(self.straight)
            rospy.loginfo(rospy.get_caller_id() + " Drive straight 1m completed, header.seq is %d", self.header.seq)
            
                    

if __name__ == '__main__':
    rospy.init_node('lab2_straight')
    lab2_straight()
